...
public class SearchResults extends ListActivity{
	...
	private void showProgressDialog() {
		downloadDialog = ProgressDialog.show( this, getString(R.string.app_name) , getString(R.string.downloading_schedules), true);

	}
	private void hideProgressDialog() {
		downloadDialog.dismiss();
	}
	private void getDataFromServer() throws JSONException, ParseException, ClientProtocolException, NotFoundException, IOException {
		...
		schedulesList = getRozklad(fromSpinnerValue, toSpinnerValue, a.getHours() + ":" +a.getMinutes(), getResources());
		if(RozkladWKD.DEBUG_LOG) {
			Log.i("TAG","from spinner:" + fromSpinnerValue+ "; to spinner: " +toSpinnerValue);
		}
	...
	private void setListAdapter() {
		...
		if(error) {
			...
			Toast.makeText(this, R.string.have_not_found_connections, 1000).show();
		} else {
			cal = Calendar.getInstance();
        	
			scheduleAdapter = new ScheduleAdapter(this, R.layout.rozklad_wkd_search_results_row, schedulesList); 
			
			setListAdapter(scheduleAdapter );
		}
	}

	/* Pobieranie danych z RozkladWKD activity o stacji poczatkowej i kierunku jazdy*/
	private void getExtras() {
		...
		if(extras != null){
			...
			if(extras.containsKey(RozkladWKD.YEAR)) {
				...
				searchTime.set(Calendar.DAY_OF_MONTH, extras.getInt(RozkladWKD.DAY));
				searchTime.set(Calendar.HOUR_OF_DAY, extras.getInt(RozkladWKD.HOUR));
				searchTime.set(Calendar.MINUTE, extras.getInt(RozkladWKD.MINUTE));
			}
		} else {
			this.finish();
		}
	...
	private class ScheduleAdapter extends ArrayAdapter<Schedule> {
	    ...
	    public ScheduleAdapter(Context context, int textViewResourceId, LinkedList<Schedule> items) {
	            ...
	            if (o != null) {
	                	...
	                	if(!showAllSchedules) {
	                		...
	                		if(!checkIfScheduleExist(searchTime, o)) {
	                			...
	                			return vi.inflate(R.layout.rozklad_wkd_search_results_null_row, null);
	                    ...
	                    if (tt != null) {
	                    	tt.setTextColor(R.color.black);
	                    	  if(o.fromTime.get(Calendar.HOUR_OF_DAY) >= 0 && o.fromTime.get(Calendar.HOUR_OF_DAY) <= 9) {
	                    		  hour = "0" + o.fromTime.get(Calendar.HOUR_OF_DAY);
	                    	  } else {
	                    		  hour = String.valueOf(o.fromTime.get(Calendar.HOUR_OF_DAY));
	                    	  }
	                    	  if(o.fromTime.get(Calendar.MINUTE) >= 0 && o.fromTime.get(Calendar.MINUTE) <= 9) {
	                    		  minute = "0" + o.fromTime.get(Calendar.MINUTE);
	                    	  } else {
	                    		  minute = String.valueOf(o.fromTime.get(Calendar.MINUTE));
	                    	  }
	                    ...
	                    if(bt != null){
	                    	bt.setTextColor(R.color.very_dark_grey);
	                    	  if(o.toTime.get(Calendar.HOUR_OF_DAY) >= 0 && o.toTime.get(Calendar.HOUR_OF_DAY) <= 9) {
	                    		  hour = "0" + o.toTime.get(Calendar.HOUR_OF_DAY);
	                    	  } else {
	                    		  hour = String.valueOf(o.toTime.get(Calendar.HOUR_OF_DAY));
	                    	  }
	                    	  if(o.toTime.get(Calendar.MINUTE) >= 0 && o.toTime.get(Calendar.MINUTE) <= 9) {
	                    ...
	                    if(et != null) {
	                    	...
	                    	if(searchTime != null) {
	                    		et.setTextColor(getResources().getColor(R.color.search_list_neutral));
	                    		et.setText((changedPosition) + ".");
	                    		changedPosition ++;
		                    	...
		                    	Long diff = o.fromTime.getTimeInMillis() - cal.getTimeInMillis();
		                    	...
		                    	if(diff < 0) {
		                    		et.setTextColor(getResources().getColor(R.color.search_list_red));
		                    		et.setText("- " + tempCal.get(Calendar.MINUTE)+ "m");
		                    		
		                    	} else if(tempCal.get(Calendar.HOUR_OF_DAY) == 1 && tempCal.get(Calendar.MINUTE) >=0 && tempCal.get(Calendar.MINUTE) <=1) {
		                    		et.setTextColor(getResources().getColor(R.color.search_list_red));
		                    		et.setText(tempCal.get(Calendar.MINUTE) + "m");	       
		                    	} else if(tempCal.get(Calendar.HOUR_OF_DAY) == 1 && tempCal.get(Calendar.MINUTE) >1 && tempCal.get(Calendar.MINUTE) <=10) {
		                    		et.setTextColor(getResources().getColor(R.color.search_list_yellow));
		                    		et.setText(tempCal.get(Calendar.MINUTE) + "m");	                    		
		                    	} else {
		                    		et.setTextColor(getResources().getColor(R.color.search_list_neutral));
		                    		if(tempCal.get(Calendar.HOUR_OF_DAY) == 1) {
		                    			...
		                    			et.setText( (tempCal.get(Calendar.HOUR_OF_DAY) - 1) + "h" + tempCal.get(Calendar.MINUTE)+ "m");
		                    		}
			...
			if(o.type == null || o.type.equals("")) {
				...
				if(o.type.equals("D")) {
					...
					if(checkIfItIs1(calendar)) {
						...
						if(calendar.get(Calendar.DAY_OF_WEEK) == 1 || calendar.get(Calendar.DAY_OF_WEEK) == 7 || isHoliday(calendar)) {
							return true; 
						} else {
							return false;
						}
		...
		private boolean checkIfItIs1(Calendar calendar) {
			if(calendar.get(Calendar.MONTH) == 6 || calendar.get(Calendar.MONTH) == 7)
				return true;
			else 
				return false;
		}
		...
		private Boolean checkIfItIsD(Calendar calendar) {
			...
			if(calendar.get(Calendar.DAY_OF_WEEK) >=2 && calendar.get(Calendar.DAY_OF_WEEK)<=6 && !isHoliday(calendar)) {
				return true;
			} else {
				return false;
			}
		...
		private boolean isHoliday(Calendar calendar) {
			if(calendar.get(Calendar.MONTH) == 0 && calendar.get(Calendar.DAY_OF_MONTH) == 1) {
				return true;
			} else if(calendar.get(Calendar.MONTH) == 4 && (calendar.get(Calendar.DAY_OF_MONTH) == 1 || calendar.get(Calendar.DAY_OF_MONTH) == 3)) {
				return true;
			} else if(calendar.get(Calendar.MONTH) == 7 && calendar.get(Calendar.DAY_OF_MONTH) == 15) {
				return true;
			} else if(calendar.get(Calendar.MONTH) == 10 && (calendar.get(Calendar.DAY_OF_MONTH) == 1 || calendar.get(Calendar.DAY_OF_MONTH) == 11)) {
				return true;
			} else if(calendar.get(Calendar.MONTH) == 11 && (calendar.get(Calendar.DAY_OF_MONTH) == 25 || calendar.get(Calendar.DAY_OF_MONTH) == 26)) {
				return true;
			} 
			return false;
		}
	...
	private LinkedList<Schedule> getRozklad(Long fromStation, Long toStation, String time, Resources resources) throws JSONException, ParseException, ClientProtocolException, NotFoundException, IOException {
		
		final boolean isLocalSchedule =settings.getBoolean(RozkladWKD.LOCAL_SCHEDULE, false);
		final boolean isNewVersionAvailable = settings.getBoolean(RozkladWKD.NEW_VERSION_AVAILABLE, false);
	...
	private LinkedList<Schedule> getOnlineRozklad(Long fromStation,
			Long toStation, String time, Resources resources) throws ClientProtocolException, JSONException, IOException, ParseException {
		LinkedList<Schedule> schedulesList = new LinkedList<Schedule>();
		SimpleDateFormat formatterDate = new SimpleDateFormat("HH:mm:ss yyyy-MM-dd");
		...
		nameValuePairs.add(new BasicNameValuePair("requestType", "QUERY_SCHEDULE"));
	    nameValuePairs.add(new BasicNameValuePair("fromStation", fromStation.toString()));  
	    nameValuePairs.add(new BasicNameValuePair("toStation", toStation.toString()));
	...
	private LinkedList<Schedule> getLocalRozklad(Long fromStation,
		...
		final int timeMin = Integer.valueOf(timeVal[0]) * 60 + Integer.valueOf(timeVal[1]);
		...
		return localDB.getRozklad(afterMidnight, fromStation, toStation, fromMin, toMin, direction);
